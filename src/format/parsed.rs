...
impl Parsed {
    ...
    pub fn to_naive_datetime_with_offset(&self, offset: i32) -> ParseResult<NaiveDateTime> {
        ...
        if let (Ok(date), Ok(time)) = (date, time) {
            ...
            let mut datetime = NaiveDateTime::from_timestamp(ts, 0).map_err(|_| OUT_OF_RANGE)?;
    ...
    pub fn to_datetime(&self) -> ParseResult<DateTime<FixedOffset>> {
        ...
        offset.from_local_datetime(&datetime).map_err(|_| OUT_OF_RANGE)
    }
    ...
    pub fn to_datetime_with_timezone<Tz: TimeZone>(&self, tz: &Tz) -> ParseResult<DateTime<Tz>> {
        ...
        if let Some(timestamp) = self.timestamp {
            ...
            let dt =
                NaiveDateTime::from_timestamp(timestamp, nanosecond).map_err(|_| OUT_OF_RANGE)?;
            guessed_offset =
                tz.offset_from_utc_datetime(&dt).map_err(|_| OUT_OF_RANGE)?.fix().local_minus_utc();
        }
        ...
        let t = tz.from_local_datetime(&datetime).map_err(|_| OUT_OF_RANGE)?;
...
mod tests {
    ...
    fn test_parsed_to_datetime() {
        ...
        let ymdhmsn = |y, m, d, h, n, s, nano, off| {
            FixedOffset::east(off).ymd(y, m, d).unwrap().and_hms_nano(h, n, s, nano).unwrap()
        };
    ...
    fn test_parsed_to_datetime_with_timezone() {
        ...
        assert_eq!(
            ...
            Ok(FixedOffset::east(32400).ymd(2014, 12, 31).unwrap().and_hms(13, 26, 40).unwrap())
        );
