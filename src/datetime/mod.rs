...
impl Default for DateTime<FixedOffset> {
    fn default() -> Self {
        FixedOffset::west(0).from_utc_datetime_fixed(&NaiveDateTime::default())
    }
}
...
impl From<DateTime<Utc>> for DateTime<FixedOffset> {
    ...
    fn from(src: DateTime<Utc>) -> Self {
        src.with_fixed_timezone(&FixedOffset::east(0))
    }
}
    ...
    F: FnMut(NaiveDateTime) -> Result<NaiveDateTime, ChronoError>,
{
    ...
    dt.timezone().from_local_datetime(&datetime)
}
...
fn test_auto_conversion() {
    let utc_dt = Utc.ymd(2018, 9, 5).unwrap().and_hms(23, 58, 0).unwrap();
    let cdt_dt =
        FixedOffset::west(5 * 60 * 60).ymd(2018, 9, 5).unwrap().and_hms(18, 58, 0).unwrap();
    ...
    E: ::core::fmt::Debug,
{
    ...
    assert_eq!(
        to_string_fixed(
            &FixedOffset::east(3660).ymd(2014, 7, 24).unwrap().and_hms(12, 34, 6).unwrap()
        )
        .ok(),
    ...
    assert_eq!(
        to_string_fixed(
            &FixedOffset::east(3650).ymd(2014, 7, 24).unwrap().and_hms(12, 34, 6).unwrap()
        )
        .ok(),
    ...
    E: ::core::fmt::Debug,
{
    ...
    assert_eq!(
        ...
        norm(&Some(FixedOffset::east(0).ymd(2014, 7, 24).unwrap().and_hms(12, 34, 6).unwrap()))
    );
    assert_eq!(
