...
fn test_datetime_date_and_time() {
    ...
    assert_eq!(d.date(), tz.ymd(2014, 5, 6).unwrap());
    ...
    assert_eq!(d.date(), tz.ymd(2016, 5, 4).unwrap());
    ...
    assert_eq!(d.date(), tz.ymd(2017, 8, 9).unwrap());
...
fn test_datetime_rfc2822_and_rfc3339() {
    ...
    assert_eq!(
        ...
        Ok(FixedOffset::east(0).ymd(2015, 2, 18).unwrap().and_hms(23, 16, 9).unwrap())
    );
    assert_eq!(
        DateTime::parse_from_rfc2822("Wed, 18 Feb 2015 23:16:09 -0000"),
        Ok(FixedOffset::east(0).ymd(2015, 2, 18).unwrap().and_hms(23, 16, 9).unwrap())
...
fn test_datetime_from_str() {
    assert_eq!(
        ...
        Ok(FixedOffset::east(0).ymd(2015, 2, 18).unwrap().and_hms_milli(23, 16, 9, 150).unwrap())
    );
    assert_eq!(
...
fn test_datetime_parse_from_str() {
    let ymdhms = |y, m, d, h, n, s, off| {
        FixedOffset::east(off).ymd(y, m, d).unwrap().and_hms(h, n, s).unwrap()
    };
    assert_eq!(
...
fn test_to_string_round_trip() {
    ...
    let ndt_fixed = dt.with_fixed_timezone(&FixedOffset::east(3600));
    let _dt: DateTime<FixedOffset> = ndt_fixed.to_string().parse().unwrap();

    let ndt_fixed = dt.with_fixed_timezone(&FixedOffset::east(0));
    let _dt: DateTime<FixedOffset> = ndt_fixed.to_string().parse().unwrap();
}
...
fn test_datetime_format_alignment() {
    let datetime = Utc.ymd(2007, 1, 2).unwrap();
