...
impl TimeZone for Local {
    ...
    fn offset_from_local_date(&self, local: &NaiveDate) -> Result<Self::Offset, ChronoError> {
        Ok(*self.from_local_date(local)?.offset())
    }
    ...
    fn offset_from_local_datetime(
        ...
        Ok(*self.from_local_datetime(local)?.offset())
    }
    ...
    fn from_local_date(&self, local: &NaiveDate) -> Result<Date<Local>, ChronoError> {
        ...
        Ok(Date::from_utc(*local, *midnight.offset()))
    }
...
mod tests {
    ...
    fn test_local_date_sanity_check() {
        // issue #27
        assert_eq!(Local.ymd(2999, 12, 28).unwrap().day(), 28);
    }
